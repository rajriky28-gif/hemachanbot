...
async function createCollage(imageUrls, gridDim = 2) {
  ...
  // Get metadata of the first image to calculate the aspect ratio
  const firstMetadata = await sharp(imageBuffers[0]).metadata();
  const aspect = (firstMetadata.width && firstMetadata.height)
    ? (firstMetadata.width / firstMetadata.height)
    : 1;

  const P = 15; // padding between images (in pixels)
  const M = 15; // outer margin/border around the grid (in pixels)
  
  // Standardize cell width based on grid dimension G to keep final collage size controlled
  let W_cell = 600;
  if (G === 2) {
    W_cell = 800;
  } else if (G === 3) {
  ...
  for (let idx = 0; idx < K; idx++) {
    ...
    // Since the cell aspect ratio matches the image aspect ratio, there is no cropping.
    const resizedBuf = await sharp(buf)
      .resize(W_cell, H_cell, { fit: 'cover' })
      .toBuffer();
