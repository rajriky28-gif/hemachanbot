...
async function createCollage(imageUrls, direction = 'horizontal') {
  ...
  // Adjust resolution dynamically to stay within Vercel's serverless timeout and memory limits
  let targetDimension = 2000;
  if (imageUrls.length >= 7) {
    targetDimension = 800; // Safe low resolution for large batches
  } else if (imageUrls.length >= 5) {
    targetDimension = 1200; // Medium resolution
  } else if (imageUrls.length >= 3) {
    targetDimension = 1600; // High resolution
  }
